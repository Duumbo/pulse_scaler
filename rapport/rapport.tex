...
\usepackage[french]{babel} % Pour que les mots-clés s'affichent en français.
\usepackage{amsmath,amssymb} % Pour les mathématiques.
\usepackage{esint} % Pour d'autres mathématiques
\usepackage{physics} % Quelques raccourcis utiles pour la physique, dont la commande \qty que j'adore.
\usepackage{esvect}
\usepackage[margin=2.5cm]{geometry} % Les marges par défaut sont énormes.
\usepackage{multicol} % Pour avoir des colonnes.
\usepackage{siunitx} % Pour les unités de mesures.
\usepackage{graphicx} % Pour les figures.
\usepackage{hyperref} % Permet de cliquer pour suivre les références.
\usepackage{subfig}
\newcommand{\p}{\partial}
